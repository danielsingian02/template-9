<?php
/**
 * template-9 functions and definitions
 *
...
function template_9_setup() {
	/*
		...
		* If you're building a theme based on template-9, use a find and replace
		* to change 'template-9' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'template-9', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'template-9' ),
		)
	);
...
function template_9_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'template-9' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'template-9' ),
...
function template_9_scripts() {
	wp_enqueue_style( 'template-9-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'template-9-style', 'rtl', 'replace' );

	wp_enqueue_script( 'template-9-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
